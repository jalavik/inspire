...
def element_tree_oai_to_records(tree, header_subs=None):
    ...
    for record_element in oai_records.getchildren():
        ...
        if "status" in header.attrib and header.attrib["status"] == "deleted":
            ...
            deleted_record = {}
            record_add_field(deleted_record, "035", subfields=[("9", "Zenodo"), ("a", recid)])
...
def apply_filter(rec):
    ...
    if not 'hidden' in [x.lower() for x in record_get_field_values(rec, "980",
                                                                   code="a")]:
        record_add_field(rec, '035', subfields=[('9', 'Zenodo'), ('a', zenodo_id)])

    # Clear control fields
    record_strip_controlfields(rec)

    # Clear other uninteresting fields
    interesting_fields = ["024", "035", "100", "245", "260", "700", "710", "773",
                          "856", "520", "500"]
    for tag in rec.keys():
    ...
    for desc in descriptions:
        ...
        if 'a' in subs:
            record_add_field(rec, "520", subfields=[('9', 'Zenodo'), ('h', subs['a'][0])])
    ...
    for field in record_get_field_instances(rec, '773'):
        ...
        if 'i' in subs and 'isSupplementTo' in subs['i']:
            if 'n' in subs and "doi" in subs['n']:
                paper_recid =  perform_request_search(p="0247_a:%s" % subs['a'][0], of="id")
                if paper_recid:
                    record_add_field(rec, "786", subfields=[('w', str(paper_recid[0]))])
            if 'n' in subs and "arXiv" in subs['n']:
                paper_recid =  perform_request_search(p="037__a:%s" % subs['a'][0], of="id")
                if paper_recid:
                    record_add_field(rec, "786", subfields=[('w', str(paper_recid[0]))])
...
def attempt_record_match(recid):
    ...
    return perform_request_search(p="035:Zenodo and 035:%s and 980:DATA" % (recid,), of="id", c="DATA")
